...
Note that build arguments are _not available_ in the runtime container. If you need build information at runtime - like a Git revision - store it in a file at build time, like:
...
To run a command in a temporary Machine _before_ the release is deployed, define a `release_command`:
